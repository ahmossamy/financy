-- Financy extended schema
-- This migration runs after 001_core_money_schema.sql.
-- Core tables, currencies, shared indexes, set_updated_at(), and core RLS
-- policies are defined by the preceding migration and are not recreated here.

-- Carry forward the additional relationships and constraints from the
-- consolidated initial schema without redefining the core tables.
...
alter table public.profiles
  add constraint profiles_base_currency_fkey
...
alter table public.exchange_rates
  add constraint exchange_rates_distinct_currencies_check
  check (from_currency <> to_currency);

alter table public.exchange_rates
  add constraint exchange_rates_unique_rate
...
alter table public.accounts
  add constraint accounts_statement_date_check
  check (statement_date is null or statement_date between 1 and 31);

alter table public.accounts
  add constraint accounts_payment_due_date_check
  check (payment_due_date is null or payment_due_date between 1 and 31);
...
alter table public.tags
  add constraint tags_name_unique_per_user
  unique (user_id, name);

alter table public.transactions
  add column scheduled_transaction_id uuid,
...
alter table public.transfers
  add constraint transfers_received_amount_check
...
alter table public.transactions
  add constraint transactions_scheduled_transaction_id_fkey
  foreign key (scheduled_transaction_id)
  references public.scheduled_transactions(id)
  on delete set null;
...
create index idx_transactions_scheduled_transaction_id
  on public.transactions(scheduled_transaction_id);
...
create index idx_recurring_rules_user_id
  on public.recurring_rules(user_id);

create index idx_recurring_rules_next_run_date
  on public.recurring_rules(next_run_date);

create index idx_scheduled_transactions_user_id
  on public.scheduled_transactions(user_id);

create index idx_scheduled_transactions_next_run_date
  on public.scheduled_transactions(next_run_date);
...
create index idx_investment_assets_user_id
...
create index idx_investment_transactions_transaction_date
  on public.investment_transactions(transaction_date);

create index idx_dividends_user_id
  on public.dividends(user_id);

create index idx_dividends_payment_date
  on public.dividends(payment_date);

create index idx_asset_prices_user_id
  on public.asset_prices(user_id);

create index idx_asset_prices_price_date
  on public.asset_prices(price_date);

create index idx_asset_valuations_user_id
  on public.asset_valuations(user_id);

create index idx_asset_valuations_valuation_date
  on public.asset_valuations(valuation_date);

create index idx_receivables_user_id
...
create index idx_liabilities_due_date
  on public.liabilities(due_date);
...
create index idx_goal_links_user_id
...
create index idx_backups_user_id
  on public.backups(user_id);

create index idx_backups_created_at
  on public.backups(created_at);
